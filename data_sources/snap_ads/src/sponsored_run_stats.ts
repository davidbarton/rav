 ...
 *
 * Usage:
 *   npx tsx src/sponsored_run_stats.ts
 *   npx tsx src/sponsored_run_stats.ts --path data/sponsored_2026-04-08T17-21-33
 *   npx tsx src/sponsored_run_stats.ts --json
 */
...
function parseArgs(): { pathArg: string | null; json: boolean } {
  ...
  for (let i = 0; i < raw.length; i++) {
    if (raw[i] === "--path" && i + 1 < raw.length) {
      ...
      console.log(`Usage: npx tsx src/sponsored_run_stats.ts [--path <sponsored_run_dir>] [--json]
