...
interface FileEntry { url: string; path: string }
...
// ── Global transparency report URLs (HTML with tables) ────

interface ReportEntry { period: string; url: string }

const GLOBAL_REPORTS: ReportEntry[] = [
...
async function scrapeGlobalReport(period: string, url: string): Promise<void> {
  ...
  const tableData: Array<{ headers: string[]; rows: Record<string, string>[] }> = [];
